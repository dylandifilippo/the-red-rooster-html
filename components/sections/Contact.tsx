        ...
        <Reveal className="grid gap-12 lg:grid-cols-2">
          <div className="space-y-8">
            <div>
              ...
              <a href={`tel:${contact.phone}`} className="mt-2 block font-serif-display text-3xl transition-colors hover:text-accent-bright">
                {contact.phoneDisplay}
              </a>
              <a href={contact.whatsappUrl} target="_blank" rel="noreferrer" className="mt-2 inline-block font-sans text-sm text-accent-bright underline">
                {t('whatsapp')} ↗
              </a>
            </div>
            <div>
              <p className="font-sans text-[10px] uppercase tracking-[0.3em] text-ink-muted">{t('addressLabel')}</p>
              <a href={contact.mapsUrl} target="_blank" rel="noreferrer" className="mt-2 block font-serif-display text-xl transition-colors hover:text-accent-bright">
